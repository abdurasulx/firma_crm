...
def get_dashboard_stats(company):
    ...
    all_products = Mahsulot.objects.filter(company=company)
    low_stock_list = [m for m in all_products if m.past_zaxira]

    stats = {
        'total_sales_today': Savdo.objects.filter(company=company, vaqt_sana__gte=today_start).aggregate(total=Sum('summa'))['total'] or 0,
        'total_sales_month': Savdo.objects.filter(company=company, vaqt_sana__gte=month_start).aggregate(total=Sum('summa'))['total'] or 0,
        'low_stock_products': len(low_stock_list),
        'low_stock_list': low_stock_list,
        'top_products_by_summa': [],
        'active_delivery_users': User.objects.filter(company=company, type='yetkazib_beruvchi', is_active=True).count()
    }
    return stats

def get_sales_timeseries(company, days=30):
