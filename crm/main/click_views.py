...
def click_prepare(request):
    if request.method == 'POST':
        ...
        try:
            payment_link = BillingPaymentLink.objects.select_related('company').get(id=int(merchant_trans_id))
        except (BillingPaymentLink.DoesNotExist, ValueError):
        ...
        if payment_link.status != 'opened' or ClickTransaction.objects.filter(merchant_trans_id=merchant_trans_id).exists():
            return JsonResponse({'error': -6, 'error_note': 'PAYMENT LINK ALREADY USED'})
...
def click_complete(request):
    if request.method == 'POST':
        ...
        if error < 0:
            ...
            tx.save()
            payment_link = BillingPaymentLink.objects.filter(id=merchant_trans_id).first()
        ...
        payment_link = BillingPaymentLink.objects.filter(id=merchant_trans_id).first()
        mark_company_paid(tx.company, now=timezone.now(), payment_link=payment_link)
        if payment_link:
